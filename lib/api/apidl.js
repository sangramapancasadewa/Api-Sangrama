...
function watuksolatmy() {
  return new Promise((resolve, reject) => {
    const date = new Date().toISOString().slice(0, 10); // mendapatkan tanggal hari ini
    const url = `https://jadwalsholat.org/jadwal-sholat/monthly.php?id=232&bulan=${date.slice(5, 7)}&tahun=${date.slice(0, 4)}`;

    axios.get(url)
      .then(response => {
        const $ = cheerio.load(response.data);
        const jadwal = {};
        $('table.sholat > tbody > tr').each((i, el) => {
          const waktu = $(el).find('td:nth-child(2)').text().trim();
          jadwal[i] = {
            imsyak: $(el).find('td:nth-child(1)').text().trim(),
            shubuh: $(el).find('td:nth-child(2)').text().trim(),
            terbit: $(el).find('td:nth-child(3)').text().trim(),
            dhuha: $(el).find('td:nth-child(4)').text().trim(),
            dzuhur: $(el).find('td:nth-child(5)').text().trim(),
            ashr: $(el).find('td:nth-child(6)').text().trim(),
            maghrib: $(el).find('td:nth-child(7)').text().trim(),
            isya: $(el).find('td:nth-child(8)').text().trim(),
          };
        });
        resolve(jadwal); // resolve dipindahkan ke dalam blok .each
      })
      .catch(error => {
        reject(error); // tambahkan penanganan kesalahan
      });
  });
}
